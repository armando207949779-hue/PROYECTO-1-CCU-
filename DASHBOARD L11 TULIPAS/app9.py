...
VERSIÓN 9.1: base completa corregida y mejorada
LÍNEA 11
"""
...
def load_data_from_sheets():
    ...
    for csv_url in urls:
        try:
            ...
            df = df.sort_values(["Fecha", "Equipo", "Formato", "Cabezal", "Tulipa"]).reset_index(drop=True)
...
def crear_heatmaps(df_filtrado):
    ...
    for idx, ((matriz, fmt, eq, subset), (row, col)) in enumerate(zip(matrices_all, positions)):
        ...
        fig.update_xaxes(title_text="Tulipa", tickfont=dict(size=7), row=row, col=col)
        fig.update_yaxes(title_text="Cabezal", tickfont=dict(size=8), autorange="reversed", row=row, col=col)
    ...
    fig.update_layout(
        ...
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(0,0,0,0)",
        margin=dict(l=60, r=90, t=100, b=50),
    )
...
def grafico_operador(df):
    ...
    dist = pd.DataFrame({"Operador": vc.index, "Cantidad": vc.values})
...
mantenciones_sel = st.sidebar.multiselect("Tipos de mantención", mantenciones_opciones, default=mantenciones_opciones)
...
if len(df_f) == 0:
    ...
    st.markdown("## Nivel 1: Visión general")
    st.plotly_chart(grafico_tendencia(df_f), use_container_width=True)

    st.markdown("---")
    st.markdown("## Nivel 2: Distribución operativa")
    ...
    st.markdown("---")
    st.markdown("## Nivel 3: Foco por personas y componentes críticos")
    ...
    st.markdown("---")
    st.markdown("## Nivel 4: Detalle específico por ubicación física")
