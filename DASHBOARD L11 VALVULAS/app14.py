...
def cargar_datos():
    ...
    df = df[df["Válvula"].between(1, 152)]
...
def grafico_por_operador(df):
    ...
    aplicar_estilo_figura(fig, "Top operadores", 320, l=170)
    fig.update_layout(
...
def grafico_turno_operador(df):
    df_tmp = df.copy()
    df_tmp["Turno"] = df_tmp["Turno"].replace("", "SIN TURNO")
    df_tmp["Operador"] = df_tmp["Operador"].replace("", "SIN OPERADOR")
...
def grafico_heatmap_valvula_mantencion(df):
    ...
    aplicar_estilo_figura(fig, "Válvulas 1-152 × tipo de mantención", 900, l=80)
    fig.update_layout(
...
def grafico_estado_valvulas(df):
    ...
    aplicar_estilo_figura(fig, "Estado global de las 152 válvulas", 380)
    fig.update_layout(
...
def grafico_valvula_mantencion_burbujas(df):
    ...
    aplicar_estilo_figura(fig, "Distribución válvula × tipo de mantención", 380)
    fig.update_layout(
        xaxis_title="Número de válvula",
...
def grafico_top_valvulas(df, top_n=15):
    ...
    top = top.sort_values("Cantidad")

    fig = go.Figure(data=[go.Bar(
        y=top["Válvula"].astype(str),
        x=top["Cantidad"],
        orientation="h",
        marker=dict(color=top["Cantidad"], colorscale="YlOrRd", showscale=False),
        text=top["Cantidad"],
        textposition="outside",
        hovertemplate="<b>Válvula %{y}</b><br>Registros: %{x}<extra></extra>"
    )])

    aplicar_estilo_figura(fig, f"Top {top_n} válvulas más intervenidas", 420, l=90)
    fig.update_layout(
...
st.markdown(
    ...
    <div style='text-align:center; margin-bottom:1.2rem;'>
        <h1>Dashboard Válvulas Krones · Línea 11</h1>
        <p style='color:#5D6D7E;'>
            Llenadora CCU · Monitoreo de mantenimiento de 152 válvulas
        </p>
    </div>
    """,
    unsafe_allow_html=True
...
if df_f.empty:
    ...
    with col_d1:
        st.download_button(
            ...
            file_name=f"resumen_valvulas_L11_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
            mime="text/csv"
        )
